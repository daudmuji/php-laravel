        ...
        <div class="col">
            <div class="card card-flush h-lg-100">
                ...
                <div class="card-body pt-5">
                    <form action="{{ route('nasabah.store') }}" class="form" method="POST" id="form">
                        @csrf
...
<script>
    ...
    $(document).ready(function () {
        $('#kode_provinsi').on('change', function () {
            $.ajax({
                url: "{{ route('stmtNasabah.get-city-by-kode-provinsi') }}?kode_provinsi=" + $(this).val(),
                type: 'GET',
                contentType: 'application/json',
                success: (data) => {
                    var html = `<option></option>`;
                    for (item of data) {
                        html += `<option value="${item.kode}">${item.kode} - ${item.nama}</option>`;
                    }
    ...
    $(document).ready(function () {
        $('#kode_kabupaten_kota').on('change', function () {
            $.ajax({
                url: "{{ route('stmtNasabah.get-kecamatan-by-kode-kota') }}?kode_kabupaten_kota=" + $(this).val(),
                type: 'GET',
                contentType: 'application/json',
                success: (data) => {
                    var html = `<option></option>`;
    ...
    $(document).ready(function () {
        $('#kode_kecamatan').on('change', function () {
            $.ajax({
                url: "{{ route('stmtNasabah.get-kelurahan-by-kode-kecamatan') }}?kode_kecamatan=" + $(this).val(),
                type: 'GET',
                contentType: 'application/json',
                success: (data) => {
                    var html = `<option></option>`;
